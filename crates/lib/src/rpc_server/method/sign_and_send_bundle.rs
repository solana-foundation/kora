//! Sign and send bundle RPC method
//!
//! Signs a bundle of transactions and submits them to the Jito block engine.
...
use crate::{
    ...
    jito::{
        ...
        types::{SignAndSendBundleRequest, SignAndSendBundleResponse},
        JitoClient,
    },
...
/// Signs and sends a bundle of transactions to Jito
pub async fn sign_and_send_bundle(
    ...
    // Sign all transactions and collect signatures
    let mut signed_encoded = Vec::with_capacity(transactions.len());
    ...
    for tx in transactions {
        ...
        // Get the fee payer's signature (first signature)
        let signature = signed_tx.signatures[0].to_string();
        signatures.push(signature);

        let encoded = TransactionUtil::encode_versioned_transaction(&signed_tx)?;
        signed_encoded.push(encoded);
    }

    // Send bundle to Jito
    let jito_client = JitoClient::new(&jito_config.block_engine_url);
    let bundle_response = jito_client.send_bundle(&signed_encoded).await?;

    Ok(SignAndSendBundleResponse { bundle_id: bundle_response.bundle_id, signatures, tip_lamports })
}
...
/// Handles tip detection and addition, returns the tip amount
fn handle_tip(
    ...
    if let Some(tip_info) = find_tip_in_bundle(transactions) {
        ...
        return Ok(tip_info.amount_lamports);
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_sign_and_send_bundle_jito_disabled() {
    ...
    fn test_validate_bundle_size_valid() {
        ...
        let transactions: Vec<String> = (0..3).map(|i| format!("tx{}", i)).collect();
