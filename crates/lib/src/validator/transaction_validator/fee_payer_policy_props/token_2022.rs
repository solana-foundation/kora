...
use spl_token_2022_interface::{
    extension::{
        metadata_pointer::instruction as metadata_pointer_ix, pausable::instruction as pausable_ix,
...
impl DrainRole for Token2022Role {
    ...
    fn instruction(self, actor: &Pubkey) -> Instruction {
        ...
        match self {
            Self::Transfer => {
                transfer_checked(&program_id, &account, &mint, &other, actor, &[], 1, 6)
                    .expect("transfer_checked")
            }
            Self::Burn => burn(&program_id, &account, &mint, actor, &[], 1).expect("burn"),
            // `other` is destination so rent/excess does not collapse back to the actor.
            Self::CloseAccount => {
                close_account(&program_id, &account, &other, actor, &[]).expect("close_account")
            }
            Self::WithdrawExcessLamports => {
                withdraw_excess_lamports(&program_id, &account, &other, actor, &[])
            ...
            Self::InitializeAccount => initialize_account3(&program_id, &account, &mint, actor)
                .expect("initialize_account3"),
            Self::InitializeMultisig => {
                initialize_multisig2(&program_id, &account, &[actor, &other], 1)
                    .expect("initialize_multisig2")
            }
            Self::FreezeAccount => {
