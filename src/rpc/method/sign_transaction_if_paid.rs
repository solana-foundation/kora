use crate::common::{
    ...
    LAMPORTS_PER_SIGNATURE, MIN_BALANCE_FOR_RENT_EXEMPTION
};
...
pub async fn sign_transaction_if_paid(
    ...
    let encoded = bs58::encode(bincode::serialize(&original_transaction)
...
fn calculate_required_lamports(transaction: &solana_sdk::transaction::Transaction) -> u64 {
    ...
    let ata_count = transaction.message.instructions
        .iter()
        .filter(|ix| transaction.message.account_keys[ix.program_id_index as usize] == ASSOCIATED_TOKEN_PROGRAM_ID)
        .count();
...
async fn validate_token_payment(
    ...
    for ix in transaction.message.instructions.iter() {
        if let Ok(spl_token::instruction::TokenInstruction::Transfer { amount }) = 
            ...
            let token_data = TokenAccount::unpack(&source_account.data)
...
async fn calculate_token_value_in_lamports(
    ...
    let mint_data = Mint::unpack(&rpc_client.get_account(mint)
        .await
        .map_err(|e| KoraError::Rpc(e.to_string()))?
        .data)
        .map_err(|e| KoraError::InvalidTransaction(format!("Invalid mint: {}", e)))?;

    let token_price_per_sig = price_info.price * pricing_params.cost_in_lamports as f64 
        / LAMPORTS_PER_SOL as f64;
    let token_price_with_margin = token_price_per_sig * (1.0 / (1.0 - pricing_params.margin));
    let token_price_in_decimal = (token_price_with_margin * 
        (10f64.powi(mint_data.decimals as i32))).floor() as u64 + 1;
