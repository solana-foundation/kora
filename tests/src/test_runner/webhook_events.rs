...
//
// These tests are run via test_runner with kora-test-webhook.toml configuration
// The test runner starts a Kora server with webhook enabled before running these tests
...
async fn test_webhook_called_on_sign_transaction() {
    ...
    // Note: The test runner starts Kora with webhook URL from config
    // We need to manually update kora-test-webhook.toml to point to this mock server
    ...
    let test_tx = ctx
        ...
        .with_spl_transfer(
            ...
            &sender.pubkey(),
            &fee_payer,
            helpers::get_fee_for_default_transaction_in_usdc(),
        )
...
async fn test_webhook_on_all_methods() {
    ...
    ctx.rpc_call::<serde_json::Value, _>(
        "transferTransaction",
        rpc_params![1_000_000, token_mint.to_string(), sender.pubkey().to_string(), recipient.to_string()],
    )
    .await
