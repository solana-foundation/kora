...
async fn test_estimate_transaction_fee() {
    ...
    let response: serde_json::Value = client
        .request("estimateTransactionFee", rpc_params![params])
        .await
        .expect("Failed to estimate transaction fee");

    assert!(
        response["fee_in_lamports"].as_u64().is_some(),
        "Expected fee_in_lamports in response"
    );
}
...
async fn test_sign_transaction() {
    ...
    assert!(
        ...
        "Expected signature in response"
    );
    assert!(
...
async fn test_sign_and_send() {
    ...
    let result = client
        .request::<serde_json::Value, _>("signAndSend", rpc_params![test_tx])
        .await;
...
async fn test_invalid_transaction() {
    ...
    let result = client
        .request::<serde_json::Value, _>("signTransaction", rpc_params![invalid_tx])
        .await;
